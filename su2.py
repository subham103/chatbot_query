...
classifier.add(Dense(output_dim = 700, init = 'uniform', input_dim = 1445))
classifier.add(LeakyReLU(alpha=0.05))
# Adding the second hidden layer
classifier.add(Dense(output_dim = 700, init = 'uniform'))
classifier.add(LeakyReLU(alpha=0.05))
...
classifier.fit(x, y, batch_size = 10, nb_epoch = 50)
...
file = open("submission_final_deeep3opps2.csv", "w")
file.write("index,tag \n") 
for k in range(425):
    file.write(str(y_sub[k]) + "," + str(decoded_y_pred[k]) + "\n")
